...
import { findFinancerAgents } from "../../../../../redux/slices/users";
          ...
          const json = await response.json()
           ...
           if(response.ok){
            ...
            const filterByRole =  json.filter((Users) => {
             ...
             || auth.user.role.description === "accountant" ? Users.role.description ==='Financer': 
            Users.role.description ==='Financer' && Users.referrer.id === auth.user.id
             })
      ...
      <MDBContainer fluid className="px-0 afl-table-container h-100">
        <MDBContainer fluid className="p-2">
          <div className="form-group position-relative">
            <MDBIcon fas icon="search" className="afl-search-icon" size="lg" />
            <input
              type="text"
        ...
        >
          {/* <MDBBtn className="afl-view-btn" onClick={handleViewAll}>
            <MDBIcon fas icon="eye" /> VIEW ALL
          </MDBBtn> */}
          {/* <MDBBtn
            className="afl-refresh-btn"
            onClick={() => window.location.reload()}
          >
            <MDBIcon fas icon="redo-alt" /> REFRESH12
          </MDBBtn> */}
            ...
            <tbody>
              { !currentUsersDisplayed ? (
                ...
                currentUsersDisplayed && currentUsersDisplayed.map((currentUsersDisplayed) => (
                  <AgentFinListTableRow allUsers={currentUsersDisplayed} key={`tr-${currentUsersDisplayed.id}`} />
                ))
              ) : typeLetterLoading ? (
