...
import { findSubAgents } from "../../../../../redux/slices/users";
...
const AgentSubListTable = () => {
  ...
  // console.log('testing 54321', storeUsers)
 ...
 Users.username.toLowerCase().includes(typedLetter)
        ...
        const json = await response.json()
         ...
         if(response.ok){
          const filterByRole =  json.filter((Users) => {
            ...
            || auth.user.role.description === "accountant" ? Users.role.description ==='sub': Users.role.description ==='sub' && Users.referrer.id === auth.user.id
           })
  ...
  //    if(response.ok){
  //     console.log('testing222', json)
  //     const filterFinancers =  json.filter((Users) => {
  //     return Users.role.description ==='sub'
  //    })
      ...
      <MDBContainer fluid className="px-0 asl-table-container h-100">
        <MDBContainer fluid className="p-2">
          <div className="form-group position-relative">
            <MDBIcon fas icon="search" className="asl-search-icon" size="lg" />
            <input
              type="text"
              className="form-control asl-search"
        ...
        >
          <MDBBtn className="asl-view-btn" onClick={handleViewAll}>
            <MDBIcon fas icon="eye" /> VIEW ALL
          </MDBBtn>
          <MDBBtn
            className="asl-refresh-btn"
        ...
        <div className="table-responsive">
          <table className="asl-table h-100">
            ...
            {/* <tbody>
              ...
              ) : storeUsers.subAgents.length ? (
                storeUsers.subAgents.map((item, i) => (
                  <AgentSubListTableRow data={item} key={`tr-${i}`} />
                ))
              ) : (
                <tr>
                  <td colSpan={12} className="text-center">
