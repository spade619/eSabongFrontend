...
import { findMasterAgents } from "../../../../../redux/slices/users";
...
const AgentMasterListTable = () => {
 ...
 Users.username.toLowerCase().includes(typedLetter)
        ...
        const json = await response.json()
         ...
         if(response.ok){
          const filterByRole =  json.filter((Users) => {
            ...
            || auth.user.role.description === "accountant" ? Users.role.description ==='master': 
            Users.role.description ==='master' && Users.referrer.id === auth.user.id
           })
    ...
    //     return Users.role.description ==='master' 
    //    })
     
    //         setAllUsers(filterFinancers)
...
const currentUsersDisplayed = allUsers.slice(indexOfFirstUsers, indexOfLastUsers)
  ...
  return (
    <MDBCol className="px-3">
      <MDBContainer fluid className="px-0 aml-table-container h-100">
        <MDBContainer fluid className="p-2">
          <div className="form-group position-relative">
            <MDBIcon fas icon="search" className="aml-search-icon" size="lg" />
            <input
              type="text"
              className="form-control aml-search"
        ...
        >
          <MDBBtn className="aml-view-btn" onClick={handleViewAll}>
            <MDBIcon fas icon="eye" /> VIEW ALL
          </MDBBtn>
          <MDBBtn
            className="aml-refresh-btn"
...
<MDBContainer
      ...
      >
        ...
        <div className="table-responsive">
          <table className="aml-table h-100">
            <thead>
              <tr className="aml-line">
            ...
            {/* <tbody>
              ...
              ) : storeUsers.masterAgents.length ? (
                storeUsers.masterAgents.map((item, i) => (
                  <AgentMasterListTableRow data={item} key={`tr-${i}`} />
                ))
              ) : (
                <tr>
                  <td colSpan={12} className="text-center">
            ...
            <tbody>
              {!currentUsersDisplayed ? (
                ...
                currentUsersDisplayed.map((currentUsersDisplayed) => (
                    <AgentMasterListTableRow
                      allUsers={currentUsersDisplayed}
                      key={`tr-${currentUsersDisplayed.id}`}
                    />
                  ) 
                )
              ) : typeLetterLoading ? (
